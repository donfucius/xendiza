...
- `xendiza.hpp` - Public C++ entry points (both 32-bit and 64-bit template specializations).
- `xendiza_c.h` - Public C ABI declarations.
- `xendiza.cpp` - C ABI implementation.
- `xendiza_x64.hpp` - x64 decode traits/tables.
- `xendiza_x64.cpp` - x64 decode implementation.
- `xendiza_x86.hpp` - x86-32 decode traits/prefix structures.
- `xendiza_x86.cpp` - x86-32 decode implementation.
- `xendiza_common.hpp` - Shared decode tables and core structures.
- `instruction.hpp` - Operand definitions.
- `opcode_id.hpp` - Opcode identifiers.
...
- Segment override prefix parsing (`CS`/`SS`/`DS`/`ES`/`FS`/`GS`) with moffs operand reporting.
