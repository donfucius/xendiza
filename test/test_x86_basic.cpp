...
TEST(X86BasicTest, MOV_A1_FS_moffs32)
{
    ...
    EXPECT_EQ(result.instrId.operands.operand[1], Operand::fs);
    EXPECT_EQ(result.instrId.operands.operand[2], Operand::moffs32);
...
TEST(X86BasicTest, INC_41_ecx)
{
    ...
    EXPECT_EQ(result.instrId.operands.operand[0], Operand::ecx);
    EXPECT_EQ(result.length, 1);
}
...
TEST(X86BasicTest, DEC_66_49_cx)
{
    ...
    EXPECT_EQ(result.instrId.operand_count, 1);
    EXPECT_EQ(result.instrId.operands.operand[0], Operand::cx);
    EXPECT_EQ(result.length, 2);
}
