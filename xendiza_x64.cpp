    ...
    const Operand opr, const bool rex_w, const bool has_66) noexcept -> Operand
{
    if (rex_w) {
        ...
        case Operand::ecx:  return Operand::rcx;
        case Operand::edx:  return Operand::rdx;
        case Operand::ebx:  return Operand::rbx;
        case Operand::esp:  return Operand::rsp;
        case Operand::ebp:  return Operand::rbp;
        case Operand::esi:  return Operand::rsi;
        case Operand::edi:  return Operand::rdi;
        default:            return opr;
        }
    }
    if (has_66) {
    ...
    const InstructionId& instr_id, const Operand seg_operand) noexcept -> InstructionId
{
    ...
    uint8_t mem_index = 0xFF;
    for (uint8_t i = 0; i < instr_id.operand_count; ++i) {
        if (IsMemoryOperand(instr_id.operands.operand[i])) {
            mem_index = i;
            break;
        }
    }
    ...
    for (uint8_t i = with_seg.operand_count; i > mem_index; --i) {
        with_seg.operands.operand[i] = with_seg.operands.operand[i - 1];
    }
    with_seg.operands.operand[mem_index] = seg_operand;
...
auto X64Traits::Disasm(gsl::span<const uint8_t> buffer) noexcept -> DecodedInstruction
{
    ...
    case 0xEE: {
        ...
        // For mod=00,r/m=100, a SIB byte follows ModRM. If SIB.base=101,
        // disp32 is mandatory and must be added on top of kLengthTable_ModRM.
        if ((modrm & 0b11'000'111u) == 0b00'000'100u) {
            if (buffer.size() <= static_cast<size_t>(prefix_info.length + 2)) {
                return ERR_INSUFFICIENT_BUFFER;
            }
    ...
    case 0xAC: {
        ...
        const auto total_len = static_cast<uint8_t>(prefix_info.length + kLengthTable_ModRM[modrm]);
        return { total_len, { instr_id.opcode, 2, form } };
    }
