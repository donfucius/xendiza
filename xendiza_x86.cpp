...
constexpr auto kDecodeTable_1byte_opcode_32 = [] {
    ...
    table[0x40] = DTE(0x01, IID(INC, eax));
    table[0x41] = DTE(0x01, IID(INC, ecx));
    table[0x42] = DTE(0x01, IID(INC, edx));
    table[0x43] = DTE(0x01, IID(INC, ebx));
    table[0x44] = DTE(0x01, IID(INC, esp));
    table[0x45] = DTE(0x01, IID(INC, ebp));
    table[0x46] = DTE(0x01, IID(INC, esi));
    table[0x47] = DTE(0x01, IID(INC, edi));
    table[0x48] = DTE(0x01, IID(DEC, eax));
    table[0x49] = DTE(0x01, IID(DEC, ecx));
    table[0x4A] = DTE(0x01, IID(DEC, edx));
    table[0x4B] = DTE(0x01, IID(DEC, ebx));
    table[0x4C] = DTE(0x01, IID(DEC, esp));
    table[0x4D] = DTE(0x01, IID(DEC, ebp));
    table[0x4E] = DTE(0x01, IID(DEC, esi));
    table[0x4F] = DTE(0x01, IID(DEC, edi));
    return table;
}();
...
constexpr auto NormalizeOperandSize32(const Operand opr) noexcept -> Operand
{
    ...
    case Operand::rcx: return Operand::ecx;
    case Operand::rdx: return Operand::edx;
    case Operand::rbx: return Operand::ebx;
    case Operand::rsp: return Operand::esp;
    case Operand::rbp: return Operand::ebp;
    case Operand::rsi: return Operand::esi;
    case Operand::rdi: return Operand::edi;
...
constexpr auto ApplyOperandSize66(const Operand opr) noexcept -> Operand
{
    ...
    case Operand::ecx: return Operand::cx;
    case Operand::edx: return Operand::dx;
    case Operand::ebx: return Operand::bx;
    case Operand::esp: return Operand::sp;
    case Operand::ebp: return Operand::bp;
    case Operand::esi: return Operand::si;
    case Operand::edi: return Operand::di;
...
constexpr auto IsMoffsOperand(const Operand opr) noexcept -> bool
{
    switch (opr) {
    case Operand::moffs8:
...
constexpr auto InjectSegmentForMoffs(
    const InstructionId& instr_id, const Operand seg_operand) noexcept -> InstructionId
{
    if (seg_operand == Operand::noopr || instr_id.operand_count >= 3) {
        return instr_id;
    }

    uint8_t moffs_index = 0xFF;
    for (uint8_t i = 0; i < instr_id.operand_count; ++i) {
        if (IsMoffsOperand(instr_id.operands.operand[i])) {
            moffs_index = i;
            break;
        }
    }
    ...
    for (uint8_t i = with_seg.operand_count; i > moffs_index; --i) {
        with_seg.operands.operand[i] = with_seg.operands.operand[i - 1];
    }
    with_seg.operands.operand[moffs_index] = seg_operand;
    with_seg.operand_count = static_cast<uint8_t>(with_seg.operand_count + 1);
    return with_seg;
}
    ...
    DecodedInstruction result, const PrefixInfo32& prefix_info) noexcept -> DecodedInstruction
{
    ...
    if (prefix_info.has_prefix.has_seg != 0) {
        result.instrId = InjectSegmentForMoffs(
            result.instrId,
...
{
    switch (opcode2) {
    case 0xBA: return GetInstructionDescriptor_opcode_extension_0x0F_BA(modrm_byte);
    default:
        return ERR_UNDEFINED_INSTRUCTION;
    }
}
    ...
    const std::array<Operands, 256>& operand_form_table) noexcept -> DecodedInstruction
{
    ...
    const auto total_len = static_cast<uint8_t>(prefix_info.length + kLengthTable_ModRM[modrm] + immediate_size);

    const auto& [operand] = operand_form_table[modrm];
    const auto imm_operand = instr_id.operands.operand[1];
    const InstructionId resolved_id = {
        instr_id.opcode,
        3,
        Operands{{ operand[0], operand[1], imm_operand }}
    };
    ...
    -> DecodedInstruction
{
    ...
    case 0x00: {
        ...
        const auto adjusted = ApplyPrefixSizes32({ static_cast<uint8_t>(modrm_len + 1), resolved_id }, prefix_info);
    ...
    case 0x09:
        {
            const auto adjusted = ApplyPrefixSizes32({ tag.instr_len, instr_id }, prefix_info);
            return { static_cast<uint8_t>(prefix_info.length + adjusted.length), adjusted.instrId };
        }
    ...
    case 0x56: {
        ...
        resolved_id.operands.operand[1] = is_reg ? Operand::r8 : Operand::m8;
    ...
    case 0x57: {
        ...
        auto resolved_id  = instr_id;
        resolved_id.operands.operand[0] = has_66 ? Operand::r16 : Operand::r32;
        resolved_id.operands.operand[1] = is_reg ? Operand::r16 : Operand::m16;
        resolved_id.operands.operand[2] = Operand::noopr;
        resolved_id.operand_count       = 2;
        const auto total_len = static_cast<uint8_t>(prefix_info.length + modrm_len + 1);
        return { total_len, resolved_id };
    ...
    case 0x5B: {
        ...
        const auto adjusted = ApplyPrefixSizes32(
    ...
    case 0x5D: {
        ...
        const bool has_66 = prefix_info.has_prefix.has_66 != 0;
        const bool has_f3 = prefix_info.rep_byte == static_cast<uint8_t>(Prefix32::REPE);
...
auto X86Traits::Disasm(gsl::span<const uint8_t> buffer) noexcept -> DecodedInstruction
{
    ...
    case 0x00: {
        ...
        const auto adjusted = ApplyPrefixSizes32({ modrm_len, resolved_id }, prefix_info);
        return { static_cast<uint8_t>(prefix_info.length + adjusted.length), adjusted.instrId };
    }
    ...
    case 0xA1: {
        ...
        auto adjusted = ApplyPrefixSizes32({ static_cast<uint8_t>(di.length - prefix_info.length), di.instrId }, prefix_info);
        return { static_cast<uint8_t>(prefix_info.length + adjusted.length), adjusted.instrId };
    }
    ...
    case 0xEE: {
        ...
        // SIB base=5 post-correction for 1-byte opcode extension dispatch.
        if ((modrm & 0b11'000'111u) == 0b00'000'100u) {
            if (buffer.size() <= static_cast<size_t>(prefix_info.length + 2)) {
                return ERR_INSUFFICIENT_BUFFER;
            }
    ...
    case 0xAC: {
        ...
        const auto adjusted = ApplyPrefixSizes32(
            { modrm_len, { instr_id.opcode, 2, form } }, prefix_info);
        return { static_cast<uint8_t>(prefix_info.length + adjusted.length), adjusted.instrId };
    }
    ...
    case 0x09:
        {
            ...
            if (opcode >= 0xA0 && opcode <= 0xA3) {
                ...
                const uint8_t raw_len = static_cast<uint8_t>(1 + (has_67 ? 2 : 4));
                if (buffer.size() < static_cast<size_t>(prefix_info.length + raw_len)) {
                    return ERR_INSUFFICIENT_BUFFER;
                }

                const auto adjusted = ApplyPrefixSizes32({ raw_len, resolved_id }, prefix_info);
                return { static_cast<uint8_t>(prefix_info.length + adjusted.length), adjusted.instrId };
            }
